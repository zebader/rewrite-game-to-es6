    ...
    if (this.rock.checkCollisionWithPlayer(this.player2)){
      ...
      this.player2.wasHit = true;
        ...
        if(this.player2.lives === 0){
          ...
          const gameOverScreen = document.querySelector('.player-winner');
...
// Events methods ===========================================
  ...
  throwRock(event){
    const final = this.finalVectorPos;
  ...
  drawPowerLine(){
    const setRadiusModule = [Math.abs(this.finalVectorPos[0] - this.rock.initialVector[0]),Math.abs(this.finalVectorPos[1] - this.rock.initialVector[1])];
    const powerRadius = Math.floor((Math.sqrt(Math.pow(setRadiusModule[0],2) + Math.pow(setRadiusModule[1],2))));
